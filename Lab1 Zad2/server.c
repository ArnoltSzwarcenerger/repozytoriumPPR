...
int main( void ) {
    ...
    fd2 = socket( AF_INET, SOCK_STREAM, 0 );		// tworzymy nowe gniazdo
	if (fd < 0) 
        return error( 1, "socket()" );
	...
	while( ( newfd2 = accept( fd, (struct sockaddr *) &cli_addr, &socklen ) ) ){
		/* sprawdzamy, czy udalo sie nawiazac polaczenie **************/
		if( newfd2 < 0 ){	// jesli wystapil blad, to wyswietlamy o nim informacje
			perror( "accept()" );
		}else{					// a jesli bledu nie bylo...
			/* wypisujemy informacje o polaczeniu *********************/
			printf("Address: %s Port: %d\n",  inet_ntoa( cli_addr.sin_addr ), cli_addr.sin_port );
